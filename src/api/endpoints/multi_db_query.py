...
async def process_multi_database_query(
    ...
    try:
        ...
        if len(connections) > 1:
            ...
            model_used = generation_result.get("model_used", model_used)

        else:
            # Single database - DON'T pre-generate SQL
            # Let the self-correcting agent (with query planning) handle it
            conn = connections[0]
            warnings = []

            # Create a placeholder - SQL will be generated by self-correcting agent
            queries = [{
                "database_name": conn.name,
                "sql": "",  # Empty string - will be generated by agent with query planning
                "is_valid": True,
                "is_read_only": True,
            }]
