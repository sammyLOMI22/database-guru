...
class SelfCorrectingSQLAgent:
    ...
    async def _try_parallel_fixes(
        ...
        database_type: str,
        trace: "AgentTrace",
        question: str = None,  # Original user question for tool-using agent
        ...
        async def try_tool_fix():
            """Try tool-using agent (disabled - context is gathered BEFORE parallel corrections)

            The tool context is now gathered BEFORE parallel corrections start, so all strategies
            (quick_fix, learned_fix, llm_fix) can benefit from the enriched schema context.

            This is better than running tool-using as a separate parallel strategy because:
            1. Tools gather context fast (< 100ms)
            2. All strategies can use the context
            3. No duplicate LLM calls
            4. No timeout issues
            """
            return None  # Context already gathered above and added to hints
            ...
            try:
                ...
                if attempt_num == 1:
                    ...
                    if sql is None:  # Only generate if not already generated by planner
                        # Generate SQL directly without tool exploration
                        # Tool-using agent is reserved for error correction retries (see try_tool_fix in parallel corrections)
                    ...
                    # Gather tool context BEFORE parallel corrections (if tool-using enabled)
                    tool_context = ""
                    if self.enable_tool_using and self.tool_using_agent:
                        try:
                            logger.info("🔧 Gathering tool context for error correction...")
                            tool_result = await self.tool_using_agent.process(
                                ...
                                use_tools=True,
                                trace=trace,
                                context_only=True,  # Fast: just gather context, no SQL generation
                            )
                            if tool_result.success and tool_result.enriched_context:
                                tool_context = tool_result.enriched_context
                                hints = f"{hints}\n\nTool-gathered insights:\n{tool_context}"
                                logger.info(f"✅ Tool context gathered: {len(tool_context)} chars from {len(tool_result.tools_used)} tools")
                        except Exception as e:
                            logger.warning(f"Tool context gathering failed: {e}")
                    ...
                    if use_parallel_corrections:
                        ...
                        fix_result = await self._try_parallel_fixes(
                            ...
                            database_type=database_type,
                            trace=trace,
                            question=question,  # Pass original question for tool-using agent
                            session=session,
                            schema_inspector=schema_inspector,
                            schema_cache=schema_cache,
                            connection_id=connection_id,
