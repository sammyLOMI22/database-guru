...
class DynamoDBHandler(NoSQLHandler):
    ...
    async def _get_schema(self, connection, inspector, db, trace):
        ...
        trace.add_step("analysis", "Inspecting DynamoDB tables...")
        ...
        if db:
            try:
                ...
                logger.warning(f"Failed to cache DynamoDB schema: {e}")
    ...
    async def _generate_and_execute_with_retry(
        ...
        for attempt_num in range(1, MAX_RETRIES + 1):
            ...
            try:
                if attempt_num == 1:
                    partiql = await generator.generate(
                        question=question, schema=schema_str, model=model,
                        db=db, query_history_id=query_history_id,
                        chat_session_id=chat_session_id,
                    )
                else:
                    partiql = await generator.generate_with_error_context(
                        question=question, schema=schema_str,
                        previous_query=last_query, error_message=last_error,
                        model=model, db=db, query_history_id=query_history_id,
                        chat_session_id=chat_session_id,
                    )
            except Exception as e:
            ...
            last_query = partiql
            trace.add_step("generation", f"Generated: {partiql[:120]}")

            result = await executor.execute(partiql)

            if result["success"]:
                trace.add_step("success", f"Returned {result['row_count']} items")
                attempts.append({"attempt": attempt_num, "query": partiql, "success": True})

                return {
                    "success": True, "sql": partiql, "result": result,
            ...
            attempts.append({"attempt": attempt_num, "query": partiql, "error": error_msg, "success": False})

        trace.add_step("error", f"All {MAX_RETRIES} attempts failed")
        return self._build_error_result(
            f"Failed after {MAX_RETRIES} attempts. Last error: {last_error}",
            trace, sql=last_query, attempts=attempts,
        )
