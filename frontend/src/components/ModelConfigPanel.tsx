...
export function ModelConfigPanel({ config, onChange, disabled = false }: ModelConfigPanelProps) {
  ...
  const getColorClasses = (color: string) => {
    const colors: Record<string, { bg: string; border: string; text: string; icon: string; darkBg: string; darkBorder: string; darkText: string; darkIcon: string }> = {
      blue: {
        bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-700', icon: 'text-blue-500',
        darkBg: 'dark:bg-blue-900/10', darkBorder: 'dark:border-blue-800/50', darkText: 'dark:text-blue-400', darkIcon: 'dark:text-blue-400'
      },
      green: {
        bg: 'bg-green-50', border: 'border-green-200', text: 'text-green-700', icon: 'text-green-500',
        darkBg: 'dark:bg-green-900/10', darkBorder: 'dark:border-green-800/50', darkText: 'dark:text-green-400', darkIcon: 'dark:text-green-400'
      },
      purple: {
        bg: 'bg-purple-50', border: 'border-purple-200', text: 'text-purple-700', icon: 'text-purple-500',
        darkBg: 'dark:bg-purple-900/10', darkBorder: 'dark:border-purple-800/50', darkText: 'dark:text-purple-400', darkIcon: 'dark:text-purple-400'
      },
      orange: {
        bg: 'bg-orange-50', border: 'border-orange-200', text: 'text-orange-700', icon: 'text-orange-500',
        darkBg: 'dark:bg-orange-900/10', darkBorder: 'dark:border-orange-800/50', darkText: 'dark:text-orange-400', darkIcon: 'dark:text-orange-400'
      },
    };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center space-x-1 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 disabled:opacity-50"
        >
      ...
      {modelsError && (
        <div className="p-3 bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-800/50 rounded-lg text-sm text-yellow-800 dark:text-yellow-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {TASK_CONFIGS.map((task) => {
          ...
          return (
            <div
              key={task.key}
              className={`p-4 rounded-lg border ${classes.bg} ${classes.border} ${classes.darkBg} ${classes.darkBorder}`}
            >
              {/* Task Header */}
              <div className="flex items-center space-x-2 mb-3">
                <Icon className={`w-5 h-5 ${classes.icon} ${classes.darkIcon}`} />
                <span className={`font-semibold ${classes.text} ${classes.darkText}`}>{task.label}</span>
              </div>
              ...
              <div className="mb-3">
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 dark:bg-gray-800 dark:text-gray-100 disabled:dark:bg-gray-700"
                >
                ...
                <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">{task.hint}</p>
              </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        <h4 className="text-md font-semibold text-gray-900 dark:text-gray-100 flex items-center space-x-2 mb-4">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg mb-3">
          <div className="flex-1">
            <label className="font-semibold text-gray-900 dark:text-gray-100">Query Templates</label>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
          ...
          <button
            ...
            disabled={disabled}
            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors disabled:opacity-50 ${config.enable_query_templates ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'
              }`}
          >
            <span
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg">
          <div className="flex-1">
            <label className="font-semibold text-gray-900 dark:text-gray-100">Location Preprocessing</label>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
          ...
          <button
            ...
            disabled={disabled}
            className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors disabled:opacity-50 ${config.enable_location_preprocessing ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'
              }`}
          >
            <span
      ...
      {/* Info Section */}
      <div className="p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/50 rounded-lg text-sm">
